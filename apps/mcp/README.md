...
`log_media` 也接受同样的 `mood`，看完一集或一部时心情和评分记在同一条里。
想单独聊某部作品（不是记一次观看），用 `capture_entry` 并把作品的 `work_…`
id 放进 `aboutId`：动态会带上作品卡片，作品详情页和 `get_item` 也会列出它。

微信 / 微博的表情是纯文字（例如 `[加油]`、`[流泪]`），聊天软件自己把它画成
小黄脸，别处只会看到方括号。所以：

- `mood` 收到 `[流泪]` 或「平静」这类预设名时，服务端会换成对应 emoji；
- 正文照原样保存，网页显示时把认识的表情码换成 emoji（`[加油]` → 💪），
  不认识的保持原样。
