...
export const entryLinkKindSchema = z.enum(["shelf", "place", "game", "work"]);
...
export const captureEntryInputSchema = z.object({
  ...
  /** Books, records, places, games or anime/screen works this post is about. */
...
export const logMediaInputSchema = z
  .object({
    ...
    visibility: z.literal("private").default("private"),
    mediaIds: entryMediaIdsSchema,
    /** Mood emoji such as "🥹"; stored as a `mood:` tag on the log entry. */
    mood: z.string().trim().min(1).max(16).optional(),
  })
  .superRefine((value, context) => {
...
/**
 * WeChat / Weibo stickers arrive as plain text like `[加油]`: the chat app
 * draws them itself, so every other screen would show the brackets. These are
 * the closest Unicode emoji for the built-in set; unknown codes stay as typed.
 */
export const STICKER_EMOJI: Readonly<Record<string, string>> = {
  微笑: "🙂", 撇嘴: "😒", 色: "😍", 发呆: "😳", 得意: "😎", 流泪: "😢",
  害羞: "😊", 闭嘴: "🤐", 睡: "😴", 大哭: "😭", 尴尬: "😅", 发怒: "😡",
  调皮: "😜", 呲牙: "😁", 惊讶: "😲", 难过: "🙁", 酷: "😎", 冷汗: "😰",
  抓狂: "😫", 吐: "🤮", 偷笑: "🤭", 愉快: "😊", 可爱: "😊", 白眼: "🙄",
...
const STICKER_PATTERN = /\[([^\[\]\s]{1,6})\]/gu;

/** Turns `[加油]`-style sticker codes into emoji; anything else is untouched. */
export function replaceStickerCodes(text: string): string {
  return text.replace(STICKER_PATTERN, (code, name: string) => STICKER_EMOJI[name] ?? code);
}

/** Normalises what a model passes as a mood: sticker codes or preset names become emoji. */
export function normalizeMood(mood: string): string {
  const trimmed = replaceStickerCodes(mood.trim());
  return (
    MOOD_PRESETS.find((preset) => preset.label === trimmed)?.emoji ??
    STICKER_EMOJI[trimmed] ??
    trimmed
  );
}
...
export interface EntryLink {
  ...
  /** anime / screen for work links. */
  workType: "anime" | "screen" | null;
}
